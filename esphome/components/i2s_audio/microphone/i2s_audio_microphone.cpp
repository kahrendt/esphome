...
void I2SAudioMicrophone::dump_config() {
  ESP_LOGCONFIG(TAG,
                ...
                YESNO(this->pdm_), YESNO(this->correct_dc_offset_));
  // "Microphone:\n"
  // "  Pin: %d\n"
  // "  PDM: %s\n"
  // "  DC offset correction: %s",
...
void I2SAudioMicrophone::configure_stream_settings_() {
  ...
  if (this->std_slot_mask_ == I2S_STD_SLOT_BOTH) {
    channel_count = 2;
  }
  // #endif
  ...
  } else if ((this->bits_per_sample_ > 16) && (this->bits_per_sample_ <= 32)) {
  ...
  this->audio_stream_info_ = audio::AudioStreamInfo(this->bits_per_sample_, channel_count, this->sample_rate_);
}
...
//   this->configure_stream_settings_();  // redetermine the settings in case some settings were changed after
//   compilation
...
void I2SAudioMicrophone::mic_task(void *params) {
  ...
  // this_microphone->parent_->enable_rx_handle();
  ...
  // this_microphone->parent_->disable_rx_handle();
...
size_t I2SAudioMicrophone::read_(uint8_t *buf, size_t len, TickType_t ticks_to_wait) {
  ...
  i2s_chan_handle_t rx_handle = this->parent_->get_rx_handle();
  esp_err_t err = i2s_channel_read(rx_handle, buf, len, &bytes_read, pdTICKS_TO_MS(ticks_to_wait));
  // #endif
  ...
  if (this->bits_per_sample_ <= 16 && !this->pdm_) {
...
void I2SAudioMicrophone::loop() {
  ...
  switch (this->state_) {
    case microphone::STATE_STARTING:
      ...
      this->configure_stream_settings_();
      this->parent_->setup_rx_handle(this->audio_stream_info_, this->audio_stream_info_.get_bits_per_sample(),
                                     this->std_slot_mask_);
