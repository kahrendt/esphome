  ...
  while (state == FileDecoderState::MORE_TO_PROCESS) {
    ...
    if (!this->pause_output_) {
      ...
      if (frames_adjustment > 0) {
        if ((this->output_transfer_buffer_->free() >= bytes_per_frame) &&
            (this->output_transfer_buffer_->available() >= bytes_per_frame)) {
          printf("duplicating a frame\n");
          std::memcpy((void *) this->output_transfer_buffer_->get_buffer_end(),
                      (void *) this->output_transfer_buffer_->get_buffer_end() - bytes_per_frame, bytes_per_frame);
          this->output_transfer_buffer_->increase_buffer_length(bytes_per_frame);
          --frames_adjustment;
        }
      } else if (frames_adjustment < 0) {
        if (this->output_transfer_buffer_->available() >= bytes_per_frame) {
          printf("deleting a frame\n");
          this->output_transfer_buffer_->decrease_buffer_length(bytes_per_frame);
          ++frames_adjustment;
        }
      }
