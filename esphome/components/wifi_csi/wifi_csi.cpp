...
void WiFiCSIComponent::update() {
  ...
  for (int i = 0; i < 52; ++i) {
    ...
    statistics::Aggregate new_aggregate = statistics::Aggregate(stats_conf);
...
void WiFiCSIComponent::wifi_csi_init_() {
...
void WiFiCSIComponent::wifi_csi_rx_callback(void *ctx, wifi_csi_info_t *info) {
  if (!info || !info->buf) {
  ...
  statistics::Aggregate temp_aggregate = statistics::Aggregate(stats_conf);
...
void WiFiCSIComponent::wifi_ping_router_start_() {
